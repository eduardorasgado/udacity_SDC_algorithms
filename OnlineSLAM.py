...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        #integrate the measurements
        for i in range(len(measurement)):
            #m is tje index of the ladmark coordinate in the matrix/vector
        ...
        #
        #x_om = len(Omega.value)
        #y_om=len(Omega.value[0])
        
        #print("Omega:{0},{1}".format(x_om,y_om))
        #print("############")
        xx = len(Omega.value)+2
        x_list = [i for i in range(4,xx)]
        yy = len(Omega.value[0])+2
        y_list = [i for i in range(4,yy)]
        Omega = Omega.expand(xx,yy,[0,1]+x_list,[0,1]+y_list)
        #Omega.show()
        #x_om = len(Omega.value)
        #y_om=len(Omega.value[0])
        #print("Omega:{0},{1}".format(x_om,y_om))
        ...
        #print(Xi)
        #print("----")
        Xi = Xi.expand(len(Xi.value)+2,1,[0,1]+[i for i in range(4,len(Xi.value)+2)],[0])
        #print(Xi)
        #updagte those spaces
        for b in range(4):
        ...
        cutted = [i for i in range(2,len(Omega.value[0]))]
        
        A =Omega.take([0,1],cutted)
        B = Omega.take([0,1])
        
        cutted2 = [i for i in range(2,len(Omega.value))]
        Atrans =Omega.take(cutted2,[0,1])
        C = Xi.take([0,1],[0])
        #A.show()
        #print("--------")
        #Atrans.show()
        ...
        lambda1= Atrans*B.inverse()*A
        lambda2 = Atrans*B.inverse()*C
        #x_lambda = len(lambda1.value)
        #y_lambda=len(lambda1.value[0])
        
        Omega = Omega.take(cutted2)-lambda1
        Xi = Xi.take([i for i in range(2,len(Xi.value))],[0])-lambda2
        x_om = len(Omega.value)
        y_om=len(Omega.value[0])
    
        print("Omega:{0},{1}".format(x_om,y_om))
        x_ee = len(Xi.value)
        y_oee=len(Xi.value[0])
    
        print("E:{0},{1}".format(x_ee,y_oee))
        
        #print("Yep!")
        #Omega.show()
    mu = Omega.inverse()*Xi
    print("dim of mu: ",len(mu.value))
    #
    #
