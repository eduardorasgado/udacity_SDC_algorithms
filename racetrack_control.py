...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        #Con ayuda de la funcion general del circulo y semicirculo se logra 
        #crear un vehiculo que realice vueltas en un circuito de carreras
        if self.x < radius:
            cte= sqrt((self.x-radius)**2+(self.y-radius)**2) -radius
        elif self.x >=3.0*radius:
            cte= sqrt((self.x-(radius*3.0))**2+(self.y-radius)**2) -radius
        elif self.y >= radius:
            cte = self.y -(radius * 2.0)
        else:
            cte = -self.y          
        return cte
...
def run(params, radius, printflag = False):
    ...
    plt.plot(xs,ys,'m^',label="robot")
    plt.legend(loc=4)
    #plt.grid(True)
    plt.title("Racetrack robot")
    plt.show()
    return err / float(N)
