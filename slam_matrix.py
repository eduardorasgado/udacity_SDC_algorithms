...
class matrix:
    ...
    def __add__(self, other):
        ...
        if self.dimx != other.dimx or self.dimx != other.dimx:
            print("Matrices must be of equal dimension to add")
            raise ValueError
        else:
    ...
    def __sub__(self, other):
        # check if correct dimensions
        if self.dimx != other.dimx or self.dimx != other.dimx:
            print("Matrices must be of equal dimension to subtract")
            raise ValueError 
        else:
...
def doit(initial_pos, move1, move2,z0,z1,z2):
    #Graph Slam para calcular la poscicion global y la posicion de un obstaculo en el mapping
    z = [z0,z1,z2]
    #initial position
    ...
    for i in range(2):
        #motion by dx
        omega.value[i][0] += 1 if i==0 else -1
        omega.value[i][1] += -1 if i==0 else 1
        
        omega.value[i+1][1]+=1 if i==0 else -1
        omega.value[i+1][2]+= -1 if i==0 else 1
        
        xi.value[i][0]+=-move1 if i==0 else move1
        xi.value[i+1][0]+=-move2 if i== 0 else move2
        
    #measurement of  landmark by dl    
    omega =omega.expand(4,4,[0,1,2],[0,1,2])
    xi = xi.expand(4,1,[0,1,2],[0])
    for i in range(3):
        #L0-xi=[10,5,2] i=0,1,2
        omega.value[i][3]+=-1.0 if i<2 else -1*5.0
        omega.value[3][i]+=-1.0 if i<2 else -1*5.0
        omega.value[i][i]+=1.0 if i<2 else 1*5.0
        omega.value[3][3]+=1.0 if i<2 else 1*5.0
        xi.value[i][0]+=-z[i] if i<2 else -z[i]*5.0
        xi.value[3][0]+=z[i] if i<2 else z[i]*5.0
    
    omega.show("omega: ")
    xi.show("Xi: ")
    #cholesky descomposition is just for
    #simetric matrix and squared ones
    #formule for graph slam
    #La mejor posicion en la que las "rubber band" no se estiran
    mu = omega.inverse()* xi 
    mu.show("mu: ")
    #mu[0,1,2] son las posciciones de nuestro robot
    #y mu[3] es la poscicion de nuestro landmark
    return mu
